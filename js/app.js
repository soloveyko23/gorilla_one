    ...
    (() => {
        ...
        class ShellGame {
            constructor() {
                ...
                this.bonusMedImage = "img/bonus/bonus-med.webp";
                this.initialImages = [];
            ...
            saveInitialImages() {
                this.initialImages = this.items.map((item => {
                    ...
                    return ballImg ? ballImg.getAttribute("data-src") || ballImg.src : "";
                }));
            }
            startGame() {
            ...
            getTransform(from, to) {
                ...
                if (isSmallScreen) return this.getTransformForSmallScreen(from, to); else return this.getTransformForLargeScreen(from, to);
            }
            getTransformForSmallScreen(from, to) {
            ...
            getTransformForLargeScreen(from, to) {
            ...
            enableItemClick() {
                this.items.forEach((item => {
                    if (item) item.addEventListener("click", this.handleItemClick.bind(this));
                }));
            }
            disableItemClick() {
                this.items.forEach((item => {
                    if (item) item.removeEventListener("click", this.handleItemClick.bind(this));
                }));
            }
            handleItemClick(event) {
                if (this.clickedOnce && this.secondClick) return;
                ...
                if (clickedItem && !clickedItem.classList.contains("item-active")) setTimeout((() => {
                    ...
                    setTimeout((() => {
                        if (!this.clickedOnce) {
                            ...
                            this.showPopupStage(1);
                        } else if (!this.secondClick) {
                            this.secondClick = true;
                            this.showPopupStage(2);
                        }
                    }), 500);
                }), 0);
            }
            ...
            endGame() {
                if (this.isSecondGame) this.resetItemPositions();
                this.clickedOnce = false;
                this.secondClick = false;
                if (this.isFirstGame) {
                ...
                } else this.isSecondGame = false;
                this.gameContainer.classList.remove("start-game", "choose-item");
                this.disableItemClick();
            }
            restoreInitialImages() {
                this.items.forEach(((item, index) => {
                    const ballImg = item.querySelector(".game__bonus-ball img");
                    if (ballImg && this.initialImages[index]) ballImg.src = this.initialImages[index];
                }));
            }
            resetItemPositions() {
                this.items.forEach(((item, index) => {
